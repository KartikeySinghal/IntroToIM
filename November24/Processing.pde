//Music taken from : https://patrickdearteaga.com/arcade-music/
...
float blue_value = 0;  
float potentiometer = 0;    // values for the state of each of the 4 buttons on the breadboard (1 or 0)
int speed;
Serial myPort;
...
void draw() {
  ...
  mapKeytoButton();
  speed = int(map(potentiometer,0,1023,0,20));
}
...
void serialEvent(Serial myPort) 
{
  ...
  if (inString != null) {
    ...
    if (button_press.length >= 5) {
      green_value = button_press[0];
      red_value = button_press[1];
      yellow_value = button_press[2];
      blue_value = button_press[3];
      potentiometer = button_press[4];
      println(speed);
    }
  }
}
...
void gameStart() {
  ...
  for (int y = 0; y<rows; y++) {  //loop through all the rows and coloumns of the grid generated above
    ...
    for (int x = 0; x<cols; x++) {
      ...
      xoff+=0.1; //increments the x-offset to determine how large the difference between each instance of noise generation should be. Since this determines the relative pulling up of vertices, it eventually determines how 
      //smooth the terrain should be or how quickly it should change
    }
    yoff+=0.1;  //increments the x-offset to determine how large the difference between each instance of noise generation should be. Since this determines the relative pulling up of vertices, it eventually determines how 
    //smooth the terrain should be or how quickly it should change
  }
